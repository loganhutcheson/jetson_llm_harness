...
SRC_DTB="/boot/dtb/kernel_tegra234-p3768-0000+p3767-0005-nv-super.imx219-c.merged.dtb"
BACKUP_DIR="/boot/dtb"
REBOOT=0
...
usage() {
  cat <<'EOF'
Usage:
  sudo ./jetson/camera/force_imx219_cam1_dtb.sh [--src-dtb PATH] [--backup-dir PATH] [--reboot]

Writes the known-good CAM1 IMX219 DTB into both Jetson kernel-dtb slots.
...
verify_dtb_strings() {
  ...
  if ! strings "${path}" | egrep -qi 'imx219|cam_i2cmux|rbpcv2'; then
    echo "expected IMX219 CAM1 markers not found in ${path}" >&2
    exit 1
  fi
}
